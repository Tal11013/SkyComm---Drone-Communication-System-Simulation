...
std::vector<double> qpsk_modulation(const std::vector<int>& data_bits, double carrier_frequency, double sampling_rate, double duration) {
    ...
    for (size_t i = 0; i < data_bits.size(); i += 2) {
        ...
        if (bit1 == 0 && bit2 == 0) phase = M_PI / 4;      
        else if (bit1 == 0 && bit2 == 1) phase = 3 * M_PI / 4; 
        else if (bit1 == 1 && bit2 == 0) phase = 5 * M_PI / 4; 
        else if (bit1 == 1 && bit2 == 1) phase = 7 * M_PI / 4; 
...
// Function to reconstruct the telemetry data from the binary data
...
int main() {
    ...
    // Modulate
    double carrier_frequency = 10.0, sampling_rate = 30.0, duration = 0.1;
